...
# Security group
resource "aws_security_group" "prod_mdle_sg_public" {
    name        = "prod_web_public"
    ...
    tags = {
        Name        = "Security Group Mdle public"
        "terraform" : "true"
    }
}

# Security group
resource "aws_security_group" "prod_mdle_sg_private" {
    name        = "prod_web_private"
    description = "Allow http and https ports"
    vpc_id      = aws_vpc.prod_mdle_vpc.id
    ...
    tags = {
        Name        = "Security Group Mdle private"
        "terraform" : "true"
    }
}
...
resource "aws_instance" "prod_mdle_ec2_private" {
    ...
    vpc_security_group_ids = [
        aws_security_group.prod_mdle_sg_private.id
    ]
...
resource "aws_instance" "prod_mdle_ec2_public" {
    ...
    instance_type     = "t2.micro"
    availability_zone = aws_subnet.prod_mdle_az_public.availability_zone
    subnet_id         = aws_subnet.prod_mdle_az_public.id

    vpc_security_group_ids = [
        aws_security_group.prod_mdle_sg_public.id
    ]
...
resource "aws_elb" "prod_mdle_elb" {
    ...
    security_groups = [ aws_security_group.prod_mdle_sg_private.id ]
